...
function adam(opfunc, x, config, state)
   ...
   local amsgrad = config.amsgrad or false
   ...
   if amsgrad then
	   ...
	   state.maxv = state.maxv or x.new(dfdx:size()):zero()
   end
  ...
  if amsgrad then
	  ...
	  state.maxv = torch.cmax(state.maxv, state.v)
	  -- Use the max. for normalizing running avg. of gradient
	  state.denom:copy(state.maxv):sqrt():add(epsilon)
  else
